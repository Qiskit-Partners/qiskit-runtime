...
class KernelMatrix:
    ...
    def __init__(self, feature_map, backend, initial_layout=None):
        ...
        Args:
            feature_map (FeatureMap): The feature map object
            backend (Backend): The backend instance
            initial layout (list or dict): Initial position of virtual qubits on
    ...
    def construct_kernel_matrix(self, x1_vec, x2_vec, parameters=None):
        ...
        Args:
            x1_vec (numpy.ndarray): NxD array of training data or test data, where
                N is the number of samples and D is the feature dimension.
            x2_vec (numpy.ndarray): MxD array of training data or support vectors, where
                M is the number of samples and D is the feature dimension.
       ...
       """
        ...
        measurement_basis = '0' * self._feature_map._num_qubits  # pylint: disable=protected-access

        if is_identical:

            # all pairwise combos of datapoint indices
            my_product_list = list(itertools.combinations(range(len(x1_vec)), 2))
            for index_1, index_2 in my_product_list:

                circuit_1 = self._feature_map_circuit(x=x1_vec[index_1],
                                                      parameters=parameters,
                                                      name='{}_{}'.format(index_1, index_2))
                circuit_2 = self._feature_map_circuit(x=x1_vec[index_2],
                                                      parameters=parameters,
                                                      inverse=True)
            ...
            mat = np.eye(len(x1_vec), len(x1_vec))
            for experiment, [index_1, index_2] in enumerate(my_product_list):
                ...
                mat[index_2][index_1] = mat[index_1][index_2]
        else:
            for index_1, point_1 in enumerate(x1_vec):
                for index_2, point_2 in enumerate(x2_vec):

                    circuit_1 = self._feature_map_circuit(x=point_1,
                                                          parameters=parameters,
                                                          name='{}_{}'.format(index_1, index_2))
                    circuit_2 = self._feature_map_circuit(x=point_2,
                                                          parameters=parameters,
                                                          inverse=True)
                    circuit = circuit_1.compose(circuit_2)
                    circuit.measure_all()
